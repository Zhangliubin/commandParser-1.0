...
public abstract class NaturalIntRangeConverter implements IConverter<int[]> {
    final int VALUE_MIN;
    final int VALUE_MAX;

    public NaturalIntRangeConverter() {
        this(0, Integer.MAX_VALUE);
    }

    public NaturalIntRangeConverter(int MIN, int MAX) {
     ...
     */
    int[] parseRangeStrict(String param) {
     ...
     * 转换为 start-end 格式
     */
    int[] parseRange(String param, boolean strict) {
        try {
            int[] parsed;
            ...
            if (strict) {
                ...
                parsed[1] = Integer.parseInt(groups[1]);
            } else {
                if (groups.length != 2) {
                ...
                parsed = new int[2];
                parsed[0] = groups[0].length() == 0 ? VALUE_MIN : Integer.parseInt(groups[0]);
                parsed[1] = groups[1].length() == 0 ? VALUE_MAX : Integer.parseInt(groups[1]);
            }
