...
public class FileStream implements AutoCloseable, Closeable {
    ...
    public FileStream(String fileName) throws IOException {
        this(fileName, FileOptions.DEFAULT_READER);
    }
    ...
    public FileStream(IFileStream file) throws IOException {
        this.file = file;
        this.mode = FileOptions.IFILESTREAM_ACCEPT;
    ...
    private IFileStream openFile(String fileName, FileOptions mode) throws IOException {
        try {
            switch (mode) {
                ...
                default: {
                    for (int i = 1; i < FileOptions.getParallelBgzipWriters().length; i++) {
                        if (mode.equals(FileOptions.getParallelBgzipWriter(i + 1))) {
    ...
    public byte[] readAll() throws IOException {
        if (this.mode != FileOptions.BGZIP_READER) {
