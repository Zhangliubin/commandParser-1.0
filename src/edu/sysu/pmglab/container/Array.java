...
public class Array<T> implements Collection<T>, RandomAccess, Cloneable {
    Object[] cache;
    int start = 0;
    int end = 0;
     ...
     */
    public Array(boolean autoExpansion) {
     ...
     * @param autoExpansion 是否可自动扩容
     */
    public Array(int size, boolean autoExpansion) {
     ...
     * @param autoExpansion 是否可自动扩容
     */
    public Array(T[] cache, boolean autoExpansion) {
     ...
     * @param length 有效数据长度
     */
    public Array(T[] cache, int length) {
        this(cache, 0, length);
    }
     ...
     * @param length 有效数据长度
     */
    public Array(T[] cache, int length, boolean autoExpansion) {
     ...
     * @param length 有效数据长度
     */
    public Array(T[] cache, int offset, int length) {
     ...
     * @param length 有效数据长度
     */
    public Array(T[] cache, int offset, int length, boolean autoExpansion) {
     ...
     * @param cache 将缓冲数据包装为循环列表
     */
    public Array(Collection<T> cache) {
        this(cache, false);
    }
     ...
     * @param cache 将缓冲数据包装为循环列表
     */
    public Array(Collection<T> cache, boolean autoExpansion) {
     ...
     */
    public Array<T> setAutoExpansion(boolean autoExpansion) {
        synchronized (this) {
            this.autoExpansion = autoExpansion;
        }
        return this;
    ...
    public boolean add(T[] elements, int offset, int length) {
        return addAll(new Array<>(elements, offset, length));
    }
     ...
     * 添加元素
     */
    public boolean addAll(Array<T> otherQueue) {
    ...
    public boolean addAll(T[] otherQueue) {
        return addAll(new Array<>(otherQueue));
    }
    ...
    public void removeByIndex(int index) {
        ...
        if (index == 0) {
            ...
            System.arraycopy(this.cache, this.start + index + 1, this.cache, this.start + index, size() - index - 1);
            this.end--;
        } else {
     ...
     */
    @SuppressWarnings("unchecked")
    public Array<T> get(int offset, int length) {
        Assert.that(offset >= 0 && length >= 0, RuntimeExceptionOptions.NegativeValueException, "offset < 0 or length < 0");
        ...
        return new Array<>((T[]) ArrayUtils.copyOfRange(this.cache, this.start + offset, this.start + offset + length), autoExpansion);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public Array<T> subList(int offset, int length) {
        Assert.that(offset >= 0 && length >= 0, RuntimeExceptionOptions.NegativeValueException, "offset < 0 or length < 0");
        Assert.that(this.start + offset <= this.end, RuntimeExceptionOptions.ArrayIndexOutOfBoundsException, "index out of bounds");
        Assert.that(this.start + offset + length <= this.end, RuntimeExceptionOptions.ArrayIndexOutOfBoundsException, "index out of bounds");

        return new Array<>((T[]) this.cache, this.start + offset, this.end + offset + length, autoExpansion);
    }
     ...
     * @param function 函数
     */
    public <N> Array<N> apply(Function<T, N> function) {
        Array<N> out = new Array<>();
        out.start = 0;
        out.end = this.end - this.start;
        out.autoExpansion = this.autoExpansion;
        out.cache = new Object[this.end - this.start];

        for (int i = start; i < this.end; i++) {
            out.cache[i - start] = function.apply((T) this.cache[i]);
        }
     ...
     *
     * @param function 函数
     * @param inplace  是否原位改变
     */
    public <N> Array<N> apply(Function<T, N> function, boolean inplace) {
        if (inplace) {
            for (int i = this.start; i < this.end; i++) {
                this.cache[i - start] = function.apply((T) this.cache[i]);
                this.cache[i] = null;
            }
     ...
     * @param function 函数
     */
    public <T> Array<T> filter(Function<T, Boolean> function) {
        Array<T> out = new Array<>();
        out.start = 0;
        out.cache = new Object[this.end - this.start];
        int pointer = 0;
        for (int i = start; i < this.end; i++) {
            if (function.apply((T) this.cache[i])) {
                out.cache[pointer++] = (T) this.cache[i];
            }
        }
     ...
     * @param inplace  是否原位改变
     */
    public <T> Array<T> filter(Function<T, Boolean> function, boolean inplace) {
        if (inplace) {
            int pointer = 0;
            for (int i = this.start; i < this.end; i++) {
                if (function.apply((T) this.cache[i])) {
                    this.cache[pointer++] = this.cache[i];
                }
     ...
     * @return 迭代值
     */
    public T reduce(Function<Pair<T, T>, T> function) {
        Object currentValue = this.cache[this.start];
        for (int i = this.start + 1; i < this.end; i++) {
            currentValue = function.apply(new Pair<>((T) currentValue, (T) this.cache[i]));
        }
     ...
     * @return 检索值
     */
    public Array<Integer> where(Function<T, Boolean> function) {
        Array<Integer> out = new Array<>();
        for (int i = this.start; i < this.end; i++) {
            if (function.apply((T) this.cache[i])) {
                out.add(i);
            }
